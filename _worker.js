...
async function processScheduledTasks(env) {
    const now = Date.now();
    const { results } = await env.XYTJ_OUTLOOK.prepare("SELECT * FROM send_tasks WHERE next_run_at <= ?").bind(now).all();
    ...
    for (const task of results) {
        try {
            ...
            if (acc) {
                ...
                // 成功
                await env.XYTJ_OUTLOOK.prepare("UPDATE send_tasks SET success_count=success_count+1 WHERE id=?").bind(task.id).run();
            }
        } catch(e) {
            console.error(e);
        }
        ...
        if (task.is_loop) {
            ...
            await env.XYTJ_OUTLOOK.prepare("UPDATE send_tasks SET next_run_at=? WHERE id=?").bind(next, task.id).run();
        } else {
            // 非循环，删除或标记完成
